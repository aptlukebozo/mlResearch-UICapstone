"""
Poster Figure 2 (non-technical audience version):
Feature Squeezing Defense — plain-English labels.

Shows three squeezing methods:
  • Blur only      (Gaussian smooth)
  • Mild squeeze   (1-bit colour + blur, Combo-1)
  • Moderate squeeze (2-bit colour + blur, Combo-2)
These were chosen because Combo-1 restores the tiger and Combo-2 restores the car.
"""
...
# ── Load & index ────────────────────────────────────────────────────────────
df = pd.read_csv("feature_squeezing_results.csv").set_index("Filename")

# Three methods to display
METHODS = [
    ("Smooth_Conf",  "Smooth_Restored",  "Blur only"),
    ("Combo1_Conf",  "Combo1_Restored",  "Colour reduction\n+ blur  (mild)"),
    ("Combo2_Conf",  "Combo2_Restored",  "Colour reduction\n+ blur  (moderate)"),
]
MLABELS = [m[2] for m in METHODS]

data = {
    img: {
        "clean_conf": float(df.loc[img, "O_Conf"]),
        "results": [
            (float(df.loc[img, conf_col]),
             str(df.loc[img, rest_col]).strip().lower() == "true")
            for conf_col, rest_col, _ in METHODS
        ],
    }
    for img in IMAGES
}
...
fig.text(0.5, 0.98, "Can Stripping Colour Detail Remove the Hidden Attack?",
         ha="center", va="top", fontsize=20, fontweight="bold")
fig.text(0.5, 0.925,
         "We reduce colour precision and apply blur — adversarial noise hides\n"
         "in fine detail. If the AI answers correctly after squeezing, the defense worked.",
...
fig.savefig("poster_fig2_feature_squeezing.png")
print("Saved poster_fig2_feature_squeezing.png")
