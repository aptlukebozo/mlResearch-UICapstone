...
IMAGE_FOLDER = "./my_images"  # Place your images here
CSV_FILENAME = "custom_research_results.csv"

# --- 2. DATA LOADING & WRAPPER ---
def get_local_images(folder):
    """Finds all image files in the local directory."""
...
import requests # Still needed for labels
...
# --- 4. LOGGING & PLOTTING ---
def log_and_plot(orig_t, adv_t, labels, o_idx, a_idx, o_conf, a_conf, l2, filename):
    # CSV Log
    ...
    noise = np.clip(((adv - orig) * 50) + 0.5, 0, 1) # 50x gain for visibility

    fig, ax = plt.subplots(1, 3, figsize=(15, 6))
    ax[0].imshow(orig); ax[0].set_title(f"Original: {labels[o_idx]}\n{o_conf:.2%}"); ax[0].axis('off')
    ax[1].imshow(adv); ax[1].set_title(f"Adversarial: {labels[a_idx]}\n{a_conf:.2%}"); ax[1].axis('off')
    ax[2].imshow(noise); ax[2].set_title("Perturbation (50x Gain)"); ax[2].axis('off')
    plt.tight_layout()
...
if __name__ == "__main__":
    ...
    if not img_files:
        print(f"[!] No images found in {IMAGE_FOLDER}. Add some .jpg or .png files!")
        exit()
    ...
    for f in img_files:
        try:
            ...
            with torch.no_grad():
                ...
                log_and_plot(img_t, adv_img, labels, o_idx.item(), a_idx.item(), o_conf.item(), a_conf.item(), l2_dist, f)
